...
def main():
    ...
    # test=st.text_area('input text to predict')
    ...
    if(st.button('predict')):
        # ndf=df[df.columns[0]]
        dataf=pd.DataFrame()
        dataf['text']=np.array(df['v2'])
        pred=[]
        for i in range (0,len(df)):
            prediction=spam_prediction(df['v2'][i])
            pred.append(prediction)
            # dataf['prediction']=prediction
        dataf['prediction']=pred
        # dataf.to_csv(index=False).encode('utf-8')
        dataf_as_csv=dataf.to_csv(index=False).encode("utf-8")
        st.download_button(
